...
function createSlot(): PopupSlot {
  ...
  return {
    ...
    active: false, curve: 'rise-fade', startTime: 0, originY: 0, aspect: 1,
  }
}

function renderText(slot: PopupSlot, text: string, color: string, fontSize = 32) {
  ...
  slot.aspect = Math.max(1, textW / canvas.height) * 1.2
  ctx.strokeStyle = '#000'
  ctx.lineWidth = 4
...
export function emit(curve: CurveName, opts: EmitOpts) {
  ...
  const baseScale = opts.icon ? 1 : 0.5
  ...
  free.sprite.scale.set(free.aspect * baseScale, baseScale, 1)
}
...
export function PopupPool() {
  ...
  useFrame(() => {
    ...
    for (const slot of slots) {
      ...
      const baseScale = slot.aspect === 1 ? 1 : 0.5
      slot.sprite.scale.set(slot.aspect * baseScale * sc, baseScale * sc, 1)
